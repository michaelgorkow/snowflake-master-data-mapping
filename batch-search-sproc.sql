...
Source: https://github.com/Snowflake-Labs/cortex-search/blob/main/examples/03_batch_querying_from_sql/batch_sproc.sql
...
SELECT 
    CASE 
        WHEN TEXT_MATCH_SCORE >= 0.9 THEN 0.9
        WHEN TEXT_MATCH_SCORE >= 0.8 THEN 0.8
        WHEN TEXT_MATCH_SCORE >= 0.7 THEN 0.7
        WHEN TEXT_MATCH_SCORE >= 0.6 THEN 0.6
        WHEN TEXT_MATCH_SCORE >= 0.5 THEN 0.5
        WHEN TEXT_MATCH_SCORE >= 0.4 THEN 0.4
        WHEN TEXT_MATCH_SCORE >= 0.3 THEN 0.3
        WHEN TEXT_MATCH_SCORE >= 0.2 THEN 0.2
        WHEN TEXT_MATCH_SCORE >= 0.1 THEN 0.1
        ELSE 0
    END as TEXT_MATCH_BUCKET,
    COUNT(*) as COUNT
FROM a
GROUP BY TEXT_MATCH_BUCKET
ORDER BY TEXT_MATCH_BUCKET DESC;
